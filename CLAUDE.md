...
| `sec:einleitung` | `chapters/01_einleitung/einleitung.tex` | FERTIG | 3 Subsections geschrieben, `check_formalia.py` 0 FEHLER; `03_vorgehen_und_aufbau.tex` um 2 rein strukturelle Sätze ergänzt (grenzwertige ½-Seite entschärft, kein neuer Claim) — Prüfbericht-Handlungsempfehlung 3, 2026-07-23 |
...
| Prüf-Audit | `pruefbericht.md` | FERTIG | **Voll-Audit 2026-07-23, Score 100/100 — abgabereif.** Deckt den Textstand nach Runde 3 (neue Matrixspalte „Rezept-Entdeckung", geänderte Chefkoch-/leftovercooking-Wertungen). 0 FAILs in Teil-Check A–C; `check_formalia.py`/`check_bib_keys.py`/`check_status.py` = 0 FEHLER. Teil-Check D (Build) ÜBERSPRUNGEN (lualatex/biber fehlen), statisch geprüft: alle `\autoref` aufgelöst, 4 Mockup-PNG vorhanden, `main.tex` bindet Anhang + beide Verzeichnisse ein. Offen (Nutzer, lokal): Build + Seitenumfang 7–10 S. |
...
**Offen (Nutzer, lokal):** Alle drei PNG-Dateien in `images/durchfuehrung/` sind noch nicht aktualisiert (`resteria_profil.png`, `resteria_rettungsfeed.png` betroffen) — das erfordert eine Claude-Design-Session außerhalb dieses Projektordners mit den Folge-Prompts aus `sources/claude-design-nachbesserung.md`; bis dahin weichen Bild und zitierter Prompt-Text an diesen zwei Stellen voneinander ab.

**Nächster Schritt: keine Textarbeit nötig.** Verbleibend sind reine Nutzer-Schritte vor der Einreichung (siehe `pruefbericht.md` → „Vor der Einreichung"): lokalen Build fahren (`latexmk -lualatex main.tex`), Seitenumfang 7–10 S. bestätigen, LanguageTool-Durchgang, Eidesstattliche Erklärung über myCampus. Optionaler Feinschliff (grenzwertige ½-Seite in `03_vorgehen_und_aufbau.tex`, Einstiegsliteratur) ist nicht abgabekritisch.
