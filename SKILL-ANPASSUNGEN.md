...
> **Status: P1-1, P1-2, P2-1, P2-2 und P3-1 sind am 2026-07-29 umgesetzt** – **P2-3 ist noch offen.** (`plan-modus`, `schreib-modus`, `pruef-modus`, `setup-check`, `hard-rules-formal.md`, `projektstruktur.md`, `vorlagen.md`, `check_quellentreue.py`; 98 Skript-Tests grün, `check_all.py` ohne harte Funde). Die Texte bleiben als Begründung stehen – sie erklären, warum die Regeln so lauten. **P4-1 ist neu, ebenfalls am 2026-07-29 umgesetzt** (eigener Anlass: ein echter BBT-Export hat gezeigt, dass die Portabilitäts-Frage aus P2-2 real und nicht nur theoretisch ist). Offen bleibt die Liste „Bitte manuell prüfen" am Dateiende.
...
## P4-1 · Dateiname = BBT-Citekey statt `<Autor><Jahr>_<Kurztitel>` – der einzige rechnerunabhängige Bezug

**Beobachtung (2026-07-29).** P2-2 hatte die Portabilitätsfrage bereits benannt („Zoteros `file`-Feld löst außerhalb des exportierenden Rechners nie auf"), aber nur als Begründung für „sofort ablegen" behandelt – nicht als eigenes Problem. Der erste **echte** BBT-Export dieses Projekts (vorher liefen nur von Hand gepflegte Bib-Einträge) hat gezeigt, wie groß die Lücke wirklich ist: Er hat für **alle neun** Einträge den `file`-Pfad auf Zoteros absolute lokale Ablage umgeschrieben (`/home/normi/Zotero/storage/<KEY>/…`) und dabei jeden zuvor von Hand ergänzten `sources/literatur/…`-Verweis gelöscht. Der bestehende Rückfall in `check_quellentreue.py` (`_pfad_kandidat`) sucht dann denselben **Dateinamen** wie im `file`-Feld in `sources/literatur/` – der Name, den Zotero oder Anna's Archive vergeben hat (z. B. „Barker et al. - 2021 - What Nudge Techniques Work…pdf"), nicht der Name, den das Projekt für dieselbe Datei gewählt hat (`Barker2021_NudgeTechniquesFoodWasteBehaviour.pdf`). Beide Namen stimmen nur zufällig überein. Ein Test mit vollständig geleertem `file`-Feld (simuliert: anderer Rechner ohne Zotero-Zugriff) bestätigte: **Ohne diese Änderung wäre keine der neun Quellen auf einem zweiten Rechner auflösbar gewesen**, ungeachtet dessen, was in `sources/literatur/` liegt.

**Warum kein Einzelfall.** Jeder Nutzer, der von einem zweiten Gerät aus weiterarbeitet – der ausdrückliche Anlass dieser Runde –, trifft auf genau dieses Problem, sobald er das erste Mal wirklich über Zotero/BBT exportiert statt von Hand zu pflegen. Es ist kein Sonderfall der drei Runde-7-Quellen, sondern die Grundannahme des gesamten `file`-Feld-Mechanismus: Zotero schreibt **immer** einen absoluten, maschinenlokalen Pfad; das Projekt hat bisher **nie** einen davon unabhängigen, deterministischen Anker gehabt.

**Zieldateien.** `.claude/skills/_shared/scripts/check_quellentreue.py` (Kernfix) · `.claude/skills/_shared/hard-rules-formal.md` · `.claude/skills/plan-modus/SKILL.md` (Schritt 1, Zotero-Erinnerung) · `.claude/skills/setup-check/SKILL.md` (Schritt 6c) · `.claude/skills/_shared/projektstruktur.md` · `sources/literatur/*` (neun Dateien umbenannt) · `test_check_quellentreue.py`.

**Umgesetzt:**

1. **Neuer, vorrangiger Auflösungsweg in `check_quellentreue.py`:** `_projekt_pfad(key, endung, root)` sucht `sources/literatur/<citekey>.pdf` bzw. `.epub` – unabhängig vom `file`-Feld-Inhalt. `pdf_pfad()`/`epub_pfad()` probieren das **zuerst**, bevor sie überhaupt den `file`-Feld-Wert ansehen; der literale Pfad und der alte Basename-Rückfall bleiben als zweite und dritte Stufe erhalten (für Fälle, in denen noch nicht umbenannt wurde). `unreferenzierte_volltexte()` zählt `<key>.pdf`/`.epub` jetzt ebenfalls als referenziert, sonst hätte jede umbenannte Datei einen falschen Aufräumhinweis ausgelöst.
2. **Alle neun zitierten Volltexte in `sources/literatur/` auf `<citekey>.<ext>` umbenannt** (`git mv`, Historie bleibt erhalten): u. a. `barkerWhatNudgeTechniques2021.pdf`, `vittuariHowReduceConsumer2023.pdf`, `erlhoferWebsiteKonzeptionUndRelaunch2017.epub`. Zwei unzitierte Volltexte (Krug, Norman) und die alte Vittuari-Preprint-Kopie bleiben unter ihrem bisherigen Namen liegen – sie tragen keinen Key, der Rückfall ignoriert sie korrekt.
3. **Portabilität verifiziert, nicht nur behauptet:** Testlauf mit einer Kopie von `references.bib`, in der jedes `file`-Feld geleert wurde (`--bib <kopie> --alle`) – alle 19 Zitationen lösten weiterhin über die Citekey-Datei auf. Das ist der eigentliche Beweis, dass die Prüfung jetzt vom exportierenden Rechner unabhängig ist.
4. **Fünf neue Tests** in `test_check_quellentreue.py` (`TestCitekeyFallback`): Citekey-Datei löst ohne `file`-Feld auf, Citekey geht vor einem vorhandenen, aber falsch benannten Zotero-Pfad, Analogtest für `.epub`, kein Fallback ohne `key`-Argument, Citekey-Datei zählt nicht als unreferenziert. 265 Tests insgesamt grün (98 vorher + 5 neu + bestehende der anderen Skripte).
5. **Regeltexte nachgezogen**, damit die Konvention nicht nur im Code, sondern auch für den nächsten Fund einer neuen Quelle gilt: `hard-rules-formal.md` (Zitationen-Abschnitt), `plan-modus` Schritt 1 (Zotero-Erinnerung), `setup-check` Schritt 6c (Inventar + Zotero-Abgleich per exaktem Citekey-Grep statt Autor/Jahr-Heuristik) und `projektstruktur.md`.

**Bewusst nicht verfolgt: Zotero „Base Directory for linked attachments".** Das wäre der GUI-seitige Weg, Better BibTeX direkt relative statt absolute Pfade exportieren zu lassen. Nachteile gegenüber der Citekey-Lösung: erfordert eine manuelle, für jede Maschine gleich einzurichtende Zotero-Einstellung (nicht git-versioniert, nicht auditierbar), setzt „Linked File"- statt „Stored Copy"-Attachments für jeden der neun Einträge voraus (Umbau der bestehenden Zotero-Library), und ich kann das GUI-Verhalten von hier aus nicht verifizieren. Die Citekey-Lösung braucht keine Zotero-Konfiguration, ist an einer Stelle (dem Skript) verifizierbar und funktioniert unabhängig davon, ob Attachments als Stored Copy oder Linked File vorliegen.

**Aufwand.** Umgesetzt, ca. 30 Minuten (Code + 9 Umbenennungen + 5 Tests + 4 Regeldateien).
