...
1. **`03_konzept.tex:15` — Wortdopplung, durch den „täglich"-Fix derselben Session eingeführt.** Der Satz trug „in der **wiederkehrenden** Nutzung verankert" und drei Zeilen später „die monatlich **wiederkehrende** Zero-Waste-Woche". Auf „über die einmalige Nutzung hinaus verankert" umgestellt — greift zugleich die Formulierung aus `:9` auf („aus einmaliger Nutzung wiederkehrendes Verhalten zu machen").
2. **`03_konzept.tex:34` — dieselbe Fehlerklasse wie Befund 2, zwei Absätze weiter.** „für die **drei** Bereiche Mockups … vor:" gefolgt von **vier** Abbildungen. Der Bericht führte das unter „Kosmetische Hinweise" Punkt 1 samt Glättungsvorschlag; nach dem Fix von Befund 2 ist es dieselbe Ankündigungszahl-gegen-Listenlänge-Differenz und wurde mitgenommen: „…liegen für die drei Bereiche **vier** Mockups in Anhang C vor: die Startseite …, **dieselbe mit ausgeklappten gespeicherten Rezepten** …". Löst nebenbei den `DOPPELBELEGUNG`-Hinweis des Skripts auf (Startseite beschriftete zwei Objekte).
3. **`06_evaluation_reflexion.tex:3` — Propagationslücke aus dem Einzel-Stresstest vom 30.07.** Dort war `02_zielsetzung_und_verengung.tex:5` auf „Gamification-Elemente … **dürften vor allem** für Menschen einen Anreiz darstellen, denen Nachhaltigkeit bereits wichtig ist **oder die sich dafür zumindest interessieren**" abgeschwächt worden, weil Somas Gamification-Gruppe ohne Vorab-Screening rekrutiert wurde und trotzdem einen Effekt zeigte. Die Reflexion trug dieselbe Aussage unverändert im Indikativ und ohne die Interessierten-Hälfte: „weil die Gamification-Mechanik vor allem bei bereits motivierten Personen **wirkt**". Jetzt „vor allem bereits motivierte **oder zumindest interessierte** Personen ansprechen **dürfte**" — deckungsgleich mit der Einleitung.

**Geprüft und für stimmig befunden** (keine Änderung): alle Zeitangaben der Phasenplanung gegen `07_iteratives_design` (Design-Phase Woche 3–4, Beta Woche 15–17 gegen „zwei bis drei Wochen", Beta-Gruppe 10–15 an beiden Stellen, MVP-Puffer zwei Wochen an beiden Stellen) · monatliche Zero-Waste-Woche in `03_konzept` und `04_community_und_feedback` · alle 14 Zählangaben erneut nachgezählt (drei Merkmale, zwei Anforderungen, fünf Phasen, drei Risiken, drei Kategorien, drei Vertreter, sechs Plattformen, ein bis drei Vertreter, zwei Kursänderungen, eine Lehre, vier Mockups, drei Hauptbereiche) · Modalitätsgrenze geplant/erbracht in `05_phasenplanung:3` gegen `07_iteratives_design:3` und `:5` · Freemium-Trennlinie gegen alle späteren Erwähnungen · Einsparangabe in `02_zielgruppe:5` gegen Konzept und Mockup.

**Zwei bewusst nicht geänderte Grenzfälle:** „eine Plattform, die alle Hobbyköch:innen ansprechen will, verwässert ihre Kernfunktionen **zwangsläufig**" (`02_zielsetzung_und_verengung.tex:5`) — starke Absolutformulierung, aber ausgewiesene Eigenargumentation ohne Quellenanspruch, und der Prüfer verlangt ausdrücklich eigenständige Argumentation. Und „Ein früher **Stresstest** deckte das auf" (`06_evaluation_reflexion.tex:11`) — Projektvokabel ohne Erklärung, im Kontext aber als „Prüfung" lesbar.

**Nach dem Durchgang:** `check_all.py --kapitel chapters/02_durchfuehrung` 0 FEHLER · `check_quellentreue.py --datei chapters/` 20/20 OK · Build Exit 0, 0 Overfull, 0 Missing character, **Textteil weiter 10 Seiten** · 3.721 Wörter.
