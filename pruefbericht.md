...
- [Hinweis, im Delta nicht gewertet] `check_bib_hygiene.py`: 9 Einträge, 28 Hinweise, keine neue Klasse. **Wichtig für die Einordnung des Scores:** Die auffälligste Klasse — gemischte Titelschreibung, fünf Einträge in Title Case gegen drei in Sentence Case — ist **im Abgabe-Audit score-relevant (−5)**. Hier bleibt sie Arbeitsliste, weil der Delta-Umfang Teil-Check F nicht als Vollprüfung führt. Ein Score von 100/100 heißt also nicht, dass an dieser Stelle nichts mehr zu tun ist.
...
1. **Zotero-Arbeitsliste abarbeiten und neu exportieren** — der einzige Punkt mit echter Score-Wirkung im späteren Abgabe-Audit (−5 für die Sentence-Case-Klasse). Reihenfolge: Sentence Case bei den fünf Title-Case-Einträgen (Barker, Erlhofer, Sauro & Lewis, Soma, UN) samt BBT-Einstellung „Titel-Casing auf Titel anwenden" = `off`, dann `urldate` leeren (6×), URL neben DOI entfernen (5×), Sprachfeld setzen (4×), Verlagsorte leeren (3×), Artikelnummern als `tex.eid` (Barker 11099, Shen 107706). Danach einmal kompilieren.
   **Richtigstellung gegenüber einer früheren Fassung dieses Berichts:** Der Zotero-Durchgang kann die Zehn-Seiten-Grenze des **Textteils** nicht kippen. Das Literaturverzeichnis steht dahinter (ab gedruckter S. 11), und Feldänderungen an Bib-Einträgen wirken nicht auf die In-Text-Zitate zurück. Der Build danach dient der Kontrolle des Verzeichnisses selbst, nicht der Seitenzahl.
...
- `check_bib_hygiene.py`-Arbeitsliste in Zotero abarbeiten und neu exportieren, danach einmal kompilieren und das gerenderte Verzeichnis ansehen (die Seitenzahl des Textteils berührt das nicht, siehe Handlungsempfehlung 1)
