...
3. **Semantischer Teil (mein Urteil, nicht delegierbar):** Zitationen mit Status `PRÜFEN` blockweise abrufen (`--paare 8`, nicht alle auf einmal – Kontext) und je Paar entscheiden. Der Quellentext kommt als Ausschnitt um die Kernbegriffe und ist als gekürzt markiert; reicht er nicht, `--paare N --voll` oder `--seite <key> <n>` nachladen – ein OK auf zu dünnem Ausschnitt ist schlimmer als ein zweiter Aufruf. Zu entscheiden ist: Deckt die Seite die Aussage inhaltlich? Ist die Paraphrase **enger oder gleich weit** wie die Quelle? Urteil zurückschreiben mit `--verdikt <hash>=OK --notiz "<Begründung>"`. Nicht gedeckte Aussagen bleiben als Befund stehen. Der typische reale Fund ist keine Erfindung, sondern eine Paraphrase, die **schärfer** formuliert ist als die Quelle (Quantoren, Absolutaussagen) – als „Claim präzisieren" listen, nicht als Plagiatsverdacht.
   ...
   - `VOLLTEXT BESCHAFFBAR` – Buch ohne hinterlegtes PDF/EPUB, oder eine Quelle, die laut `tex.zugang` über Bibliothek oder Open Access erreichbar ist. **Beschaffungsaufgabe, kein Textmangel** – Score-Folge siehe unten.
   - `ZUGANG PRÜFEN` – Artikel hinter einer bekannten Bezahlschranke ohne Snapshot. Das Skript nennt die Reihenfolge im Befundtext: freie Fassung über OpenAlex suchen, dann IU-Bibliothek, dann Quelle ersetzen. Ergebnis in Zotero als `tex.zugang: …` festhalten, sonst wird es beim nächsten Audit erneut hergeleitet.

3a. **Bevor eine Quelle als nicht prüfbar gemeldet wird, das Dateisystem befragen.** `ls sources/literatur/` und der Abgleich der Dateinamen gegen die Bib-Keys sind billiger als jeder Befund. `NICHT PRÜFBAR`, `LIVE PRÜFEN` und `DATEI NICHT GEFUNDEN` bedeuten nur, dass das Skript den Volltext über das `file`-Feld nicht **auflösen** konnte – nicht, dass er fehlt. Liegt die Datei da, ist es ein Pfad- und kein Quellenproblem, und die Prüfung wird durchgeführt statt aufgeschoben. Das Skript hilft dabei von sich aus: Es sucht denselben Dateinamen zusätzlich in `sources/literatur/` und listet am Ende Volltexte, die kein Bib-Eintrag nennt.
...
4. **Nicht prüfbare Quellen sind ein Befund, kein Freifahrtschein – aber die Klasse entscheidet über die Maßnahme:**
   - `VOLLTEXT BESCHAFFBAR` → Nutzer-Aufgabe, typischerweise eine halbe Stunde: Volltext nach `sources/literatur/` legen, Pfad ins `file`-Feld. Erscheint in der Liste „Vor der Einreichung", **außerhalb des Abgabe-Audits ohne Score-Abzug**.
   - `ZUGANG PRÜFEN` → die drei Schritte aus dem Befundtext abarbeiten (OpenAlex → Bibliothek → Ersatz). Bleibt der Zugang aus, ist das keine Formalie, sondern eine Quelle, auf die sich die Arbeit nicht stützen darf – dann wird die tragende Aussage umgestellt, nicht die Zitation stehen gelassen.
   ...
   - `LIVE PRÜFEN` (Webquelle ohne Snapshot) → Seite abrufen, Aussage prüfen, Urteil eintragen **und** den Nutzer an die Snapshot-Pflicht erinnern (Seite als PDF nach `sources/literatur/`, Pfad über Zotero ins `file`-Feld). Ohne Snapshot ist der geprüfte Stand später nicht mehr nachweisbar.
...
6. **Im Abgabe-Audit blockierend.** Offene Befunde der Klassen WORTLAUT, ZITAT WEICHT AB, SEITE VERDÄCHTIG, SEITE AUSSERHALB, NICHT GEFUNDEN und CLAIM SCHÄRFER verhindern die Abgabe-Freigabe – **im Abgabe-Audit zusätzlich `VOLLTEXT BESCHAFFBAR` und `ZUGANG PRÜFEN`**: Dort ist eine Quelle, die niemand gelesen hat, unprüfbar, ganz gleich aus welchem Grund, und die IU-KI-Richtlinie verlangt die Verifikation am Original. Die Milderung oben gilt nur für die Zwischenaudits, damit ein noch nicht heruntergeladenes Buch nicht jedes Mal Punkte kostet – das sind genau die Fälle, die in einer Bewertung als Falschzitat oder Übernahme gelten. In früheren Audits sind sie Arbeitsaufträge mit Score-Abzug.
...
| Nicht prüfbare Quelle ohne Zugangsweg (Teil-Check G: `ZUGANG PRÜFEN`, `NICHT PRÜFBAR`) | −5 | −15 |
| Volltext beschaffbar, aber nicht abgelegt (Teil-Check G: `VOLLTEXT BESCHAFFBAR`) | 0 außerhalb des Abgabe-Audits | – |
