...
1. `latexmk -lualatex main.tex` vom Projekt-Root ausführen (nutzt `.latexmkrc`, Build-Artefakte landen in `build/`).
   ...
   **Referenz ist `.zed/build.ps1`** – dort steht dieselbe Kette produktiv im Einsatz. Keine abweichende Variante erfinden, keine Flags weglassen:

   ```
   mkdir build\pages        # bzw. New-Item -ItemType Directory -Force build\pages
   ...
   Drei LuaLaTeX-Läufe, dazwischen Biber: Der zweite Lauf löst die Zitate auf, der dritte die Verweise und Verzeichnisse.

   **Die beiden leicht zu übersehenden Stellen:**
   - **`build\pages` muss vorher existieren.** `\include{pages/...}` schreibt seine `.aux`-Dateien nach `build/pages/`, und rohes lualatex legt fehlende Unterverzeichnisse **nicht** selbst an, sondern bricht ab. Nach einem `LaTeX: Clean` läuft die Kette ohne diese Zeile auf einen Fehler, der wie ein Build-Fehler der Arbeit aussieht.
