...
3. **Ist jede Behauptung nachprüfbar – und trägt jede Quelle die Rolle, die der Satz ihr zuweist?** Kapitel mit Recherche- oder Analysecharakter ohne eine einzige Quellenangabe oder dokumentierte Erhebungsgrundlage sind ein Befund, unabhängig davon, wie plausibel der Inhalt wirkt. Eigenleistung entbindet nicht von Nachvollziehbarkeit.
   ...
   **Verifikations-Gate für unzitierte Außenweltbehauptungen (vor der Einordnung, nicht danach).** Trifft die Lesung auf eine Behauptung über die reale Welt, die **keine** Zitation trägt – Eigenname, Institution, Produkt, Zahl, Jahresangabe, Funktionsanspruch („Plattform X bietet kein Y") –, wird sie **einmal recherchiert, bevor** sie eingeordnet wird. Nicht plausibel finden und weiterlesen. Dieselbe Regel wie das Verifikations-Gate im `stresstest`, hier aber ohne die Einschränkung auf Kernargumente: Der Fehler sitzt typischerweise im Nebensatz.

   Warum genau hier: Solche Sätze sind für **jeden** anderen Prüfschritt unsichtbar. Teil-Check G sieht nur Sätze mit `\cite`, der `stresstest` nur Kernargumente, `check_autoref.py` nur interne Verweise. Betroffen ist vor allem die Textklasse, die laut `aufgabe.md` → „## Prüfer-Steuerungen" als **eigene Sichtung ohne Zitationspflicht** läuft – die Ausnahme von der Zitierpflicht befreit sie nicht von der Wahrheitspflicht. Erschwerend: Die Welt bewegt sich, der Text nicht. Ministerien werden umbenannt, Produkte bekommen Funktionen, die der Text ihnen absprach – ein Satz, der beim Schreiben stimmte, kann heute falsch sein, ohne dass sich ein Zeichen geändert hat.

   Was das Web nicht hergibt, wird **nicht** vorsorglich entschärft, sondern wandert unter „Bitte manuell prüfen" (Schritt 6) – mit der Angabe, welche Option gilt, wenn die Prüfung unterbleibt. Läuft der eigene `faktencheck`-Skill im Projekt, ist dieses Gate die Auffanglinie für das, was dessen Vorfilter nicht als Kandidat erkannt hat, keine Doppelarbeit.
   ...
   **Dieselbe Reichweitenfrage gilt für interne Verweise und für Rückgriffe auf die eigene Analyse** – nicht nur für Zitationen. „Wie die Analyse gezeigt hat“, „laut~`\autoref{sec:…}`“, „aus den Ergebnissen folgt“: Die Eigenanalyse ist die Belegquelle, die am häufigsten überdehnt wird, weil niemand sie gegenliest wie eine fremde Quelle. Zu prüfen ist dasselbe – trägt die Zielstelle den Satz in seiner jetzigen Stärke?

   **Verifikations-Gate für unzitierte Außenweltbehauptungen (Pflicht, vor der Einordnung als Befund):** Trifft die Lesung auf eine **überprüfbare Tatsachenbehauptung ohne Zitation** – Name einer Institution oder Behörde, Funktionsanspruch über ein reales Produkt, Marktlage, „Plattform X bietet kein Y“ –, wird sie **erst recherchiert, dann eingeordnet**. Nicht umgekehrt, und nicht aus dem Gedächtnis. Der Grund ist eine strukturelle Lücke, keine Nachlässigkeit: Ein Kapitel, das eigene Sichtung ist und deshalb kaum Zitationen trägt, ist für Teil-Check G unsichtbar – der prüft nur Sätze **mit** Zitation. Von der Wahrheitspflicht befreit es das nicht. Real belegt: eine Ministeriumsumbenennung nach einem Regierungswechsel und ein falsch beschriebenes Konkurrenzangebot; beide fielen nur auf, weil eigens ein Faktencheck angefordert wurde – kein PASS irgendeines Teil-Checks hätte sie verhindert. Die Kandidaten markiert `check_formalia.py` in Sichtungskapiteln als AUSSENWELT-Block; die Recherche selbst bleibt hier.
   ...
   Die sieben Achsen gelten ebenso für **interne** Stützen: `\autoref`-Verweise und Rückgriffe auf die eigene Analyse („wie die Wettbewerbsanalyse gezeigt hat", „laut Kapitel X"). In Konzept- und Projektarbeiten ist die Eigenanalyse die Hauptbelegquelle und damit die Hauptquelle für Überdehnungen – anders als eine Quelle bewegt sie sich außerdem mit jeder Kürzungsrunde. Auch hier gilt: Deckung und Reichweite sind zwei Fragen.

   **Hedging-Historie ist ein Abriss-Signal, kein Politur-Signal.** Trägt ein Satz Spuren mehrfacher Abschwächung – gestaffelte Einschränkungen, „tendenziell", „zumindest teilweise", ein Beleg, der nur noch „u. a." stützt –, ist die nächste Abschwächung die falsche Empfehlung. Ein Satz, dessen Stütze in zwei oder mehr Runden nachgegeben hat, ist ein **Ersetzungs-, kein Umformulierungskandidat**: Die Zitation trägt das Argument dann ohnehin nicht mehr, und jede weitere Hedging-Runde verlängert nur die Kette. Der Befund lautet dann „Aussage neu begründen oder streichen", nicht „vorsichtiger formulieren". (Die Reparaturhistorie selbst steht in `AENDERUNGEN.md` und ist für diese Lesung Leseverbot – erkennbar ist das Muster am Satz.)
