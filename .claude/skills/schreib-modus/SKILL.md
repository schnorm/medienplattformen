...
3b. **Tragende Belege vor dem Formulieren am Volltext lesen – nicht erst danach prüfen.** Betroffen sind die Belege, an denen das **Kernargument** der Subsection hängt, sowie alle Fundstellen, die der Plan als `[zitiert: <key>, S. ?]` markiert (Fragezeichen = Seitenzahl war geschätzt). Für jeden davon die zitierte Seite ziehen und den Satz **aus dieser Seite heraus** formulieren:
   ...
   **Fehlt der Volltext, wird der Satz nicht aus dem Abstract formuliert.** Der Plan führt zu jedem `[zitiert]`-Beleg den Volltext-Status (`plan-modus` → Schritt 3); steht dort etwas anderes als „liegt vor", gibt es genau drei zulässige Wege: **(1)** Volltext jetzt beschaffen – bei `beschaffbar` ist die Route im Plan benannt, bei Büchern und Bibliothekszugängen ist das ein Nutzer-Schritt, also fragen statt umgehen; **(2)** das Argument auf einen Beleg stellen, dessen Volltext vorliegt; **(3)** die Aussage auf das zurücknehmen, was ohne diesen Beleg gedeckt ist. Aus dem Abstract zu schreiben ist der Entstehungsweg des nicht gedeckten Claims, den Teil-Check G später als `CLAIM SCHÄRFER` meldet – dann aber mit fertigem Text ringsum. Meldet `check_quellentreue.py` für den Key `VOLLTEXT BESCHAFFBAR` oder `ZUGANG PRÜFEN`, ist genau dieser Fall eingetreten.
