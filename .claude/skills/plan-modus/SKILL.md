...
### Beschaffbarkeits-Gate – vor der Priority Reading Order

**Bevor ein Treffer in die Leseliste kommt, wird geklärt, ob sein Volltext erreichbar ist.** Consensus indexiert über Metadaten; Beschaffbarkeit ist dort kein Suchkriterium, und der Qualitätsfilter (`sjr_max`) arbeitet sogar dagegen – je höher das Ranking, desto wahrscheinlicher die Bezahlschranke. Solange die Regel „Keine Zitation ohne prüfbaren Volltext" erst im Audit greift, fällt eine unerreichbare Quelle genau dann auf, wenn das Argument bereits auf ihr steht: der teuerste denkbare Zeitpunkt.

Routen in dieser Reihenfolge, es gewinnt die erste, die trägt:

1. **Liegt der Volltext schon in `sources/literature/`?** Das Listing dazu entsteht ohnehin am Anfang dieses Schritts.
2. **Freie Fassung über die DOI ermitteln – maschinell, nicht geschätzt.** Nur für Kandidaten, die tatsächlich in die engere Auswahl kommen und bei denen Route 1 nicht greift; zwanzig Abfragen für drei zitierte Quellen sind Aufwand ohne Ertrag. OpenAlex, ohne Schlüssel und ohne Anmeldung:

   ```bash
   curl -s "https://api.openalex.org/works/doi:<DOI>" | python -c "import sys,json; d=json.load(sys.stdin); print(d['open_access']); [print(l['is_oa'], l['version'], l.get('pdf_url') or l.get('landing_page_url')) for l in d['locations']]"
   ```

   **Achtung Fassung:** `submittedVersion`/`acceptedVersion` sind Preprints mit eigener Seitenzählung. Dann greift „Gelesene Fassung ≠ zitierte Fassung" (`hard-rules-formal.md`) – Fassung im Plan vermerken und `[Abs. X]`/`[Kap. X]` zitieren, solange die Verlagsseite nicht verifiziert ist. Ohne diesen Vermerk verwandelt die OA-Route einen kleinen Befund nur in einen größeren (`SEITE AUSSERHALB`).
3. **IU-Bibliothek über myCampus.** Eine Bezahlschranke im offenen Web sagt nichts über die Hochschullizenz. Das kann ich nicht prüfen – also wird **gefragt**, per `AskUserQuestion` mit der DOI in der Frage, nicht angenommen.
4. **Bücher und E-Books: Beschaffung durch den Nutzer.** Der Regelfall, und ausdrücklich kein Ausschlussgrund.

**Urteil je Quelle**, verbindlich im `[INSIGHT: literaturrecherche]`-Block und als eigene Spalte der Priority Reading Order:

- **VOLLTEXT LIEGT VOR** – uneingeschränkt zitierbar.
- **BESCHAFFBAR (`<Route>`)** – Weg benannt und zugewiesen. Kommt in den Plan, aber mit Frist: **Der Volltext liegt vor, bevor das Kapitel geschrieben wird, das ihn trägt.**
- **KEIN ZUGANG** – keine Route gefunden. **Blocker für die Auswahl**: kommt nicht in die Leseliste, bekommt kein `[zitiert]`. Ersatz wird **in derselben Session** gesucht, solange der Suchkontext steht. Zwei Monate später kostet dieselbe Entscheidung das Umschreiben einer belegten Passage.

Output an Nutzer: Priority Reading Order (3–5 Papers, geordnet), pro Paper Titel/Autoren/Journal/Jahr/Citation-Count/1-Satz-Relevanz **plus Volltext-Urteil**, Forschungslücken.

**Zotero-Erinnerung (nicht überspringen, eigene Zeile im Chat, nicht nur Nebensatz)**: Gefundene Paper jetzt in Zotero importieren und Better BibTeX exportieren lassen, damit `references.bib` rechtzeitig vor Schreib-Modus bereitsteht. Ohne diesen Schritt fehlen dort die BBT-Keys für `\parencite{}`, und Schreib-Modus darf laut eigener Regel keine Keys erfinden. **Für den BBT-Key genügen Metadaten – für die Prüfbarkeit nicht:** Wer eine Quelle gerade offen hat, legt den Volltext sofort als `sources/literature/<citekey>.pdf` ab (bei Webquellen den PDF-Ausdruck der Seite). Zoteros eigenes `file`-Feld reicht dafür nicht – es zeigt auf die lokale Ablage und löst außerhalb dieses Rechners nie auf.
...
- **(a) zitiert** – geht in `\parencite`, braucht einen BBT-Key in `references.bib`. **Mit Volltext-Urteil aus dem Beschaffbarkeits-Gate**: `[zitiert: <key>, Volltext: liegt vor]` bzw. `[zitiert: <key>, Volltext: beschaffbar – Bibliothek]`. Das Urteil muss die Session überleben; alles andere hier, was Sessions überdauert, steht in einer Datei, und ein Zugangsurteil im Gedächtnis wird beim nächsten Start neu hergeleitet oder vergessen. Bei einer Repositoriumsfassung zusätzlich die **gelesene Fassung** notieren (Verlag/Preprint) – ohne sie wandert eine Preprint-Seitenzahl ungeprüft in den Text.
