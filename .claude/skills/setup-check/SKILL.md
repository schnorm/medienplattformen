...
6c. **`sources/literatur/` – eigene Literatur inventarisieren**: Verzeichnis-Listing von `sources/literatur/` (ohne `README.md`). Dieser Ordner ist die Ablage, in die der Nutzer **schon vor dem Setup** eigene Quellen legen darf (Ebooks, Paper, Buchkapitel) – Anleitung und Namenskonvention stehen in `sources/literatur/README.md`. Ergebnis: Anzahl + Dateinamen im Statusbericht ausweisen, damit sichtbar ist, worauf Plan-Modus Schritt 1 aufsetzen kann. Dateien **nicht inhaltlich einlesen** – das ist Aufgabe des Literaturschritts, nicht des Setups.
