...
6c. **`sources/literatur/` – eigene Literatur inventarisieren**: Verzeichnis-Listing von `sources/literatur/`. Dieser Ordner ist die Ablage, in die der Nutzer **schon vor dem Setup** eigene Quellen legen darf (Ebooks, Paper, Buchkapitel, PDF-Snapshots von Webquellen); Namenskonvention **BBT-Citekey** (`<citekey>.pdf`/`.epub`, z. B. `vittuariHowReduceConsumer2023.pdf`) – nicht `<Autor><Jahr>_<Kurztitel>`, siehe `hard-rules-formal.md` und `SKILL-ANPASSUNGEN.md` P4-1. Vor dem Zotero-Import (Key steht noch nicht fest) ist ein Arbeitstitel als Dateiname übergangsweise normal. **Schreibweise beachten:** deutsch `literatur/`, nicht `literature/` – `check_quellentreue.py` akzeptiert zwar beide, aber zwei Ordner nebeneinander sind der sichere Weg zu einem Volltext, den später niemand findet. Ergebnis: Anzahl + Dateinamen im Statusbericht ausweisen, damit sichtbar ist, worauf Plan-Modus Schritt 1 aufsetzen kann. Dateien **nicht inhaltlich einlesen** – das ist Aufgabe des Literaturschritts, nicht des Setups.
   **Zotero-Abgleich (der eigentliche Zweck des Checks):** Trägt der Dateiname bereits einen Citekey, per exaktem Grep gegen `references.bib` prüfen, ob der Key dort als `@`-Eintrag existiert (`grep -c "^@.*<dateiname ohne endung>" references.bib`). Trägt er noch keinen Citekey (Arbeitstitel, `<Autor><Jahr>_...`-Altbestand), ersatzweise Autor und Jahr aus dem Dateinamen grep'en. Fehlt ein Treffer, als Hinweis listen – ohne Zotero-Eintrag gibt es keinen BBT-Key, und `schreib-modus` darf laut eigener Regel keinen erfinden; das Werk wäre also lesbar, aber nicht zitierbar. Kein Fehler, nur ein Hinweis.
