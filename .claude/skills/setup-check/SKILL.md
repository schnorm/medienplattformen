...
1. **LaTeX-Kompilierbarkeit**: prüfen, ob `main.tex` vorhanden ist und ob `lualatex` **und** `biber` als Befehle verfügbar sind (biber wird für das Literaturverzeichnis und Prüf-Modus Teil-Check D gebraucht).
   **Shellneutral prüfen**: `which` gibt es unter Windows-PowerShell nicht. Stattdessen `lualatex --version` und `biber --version` aufrufen und am Exit-Code/der Ausgabe erkennen, ob es funktioniert – das klappt unter PowerShell, cmd und POSIX-Shells gleichermaßen.
   **`scripts/build.ps1` und `scripts/clean.ps1` vorhanden?** Teil-Check D ruft `build.ps1` auf; fehlt es (etwa weil der Projektordner unvollständig kopiert wurde), aus der Vorlage nachziehen und melden. Ein Probelauf gehört **nicht** hierher – kompiliert wird erst, wenn es etwas zu kompilieren gibt. `latexmk` ist optional: bequem, wo vorhanden, aber als Perl-Skript unter MiKTeX oft nicht lauffähig – deshalb hängt Teil-Check D nicht daran.
   Falls `lualatex` oder `biber` fehlt: Nutzer auf lokale Installation hinweisen, nicht versuchen, LaTeX selbst zu installieren.
...
5. **`pages/meta.tex` – Metadaten aktiv abfragen und eintragen** (nicht nur erinnern): Datei lesen und prüfen, welche CAPS-Platzhalter noch stehen. Sind welche offen, die fehlenden Angaben **per `AskUserQuestion` im Dialog erheben und anschließend selbst per Edit in `pages/meta.tex` eintragen** – `meta.tex` ist ausdrücklich editierbar (im Gegensatz zu `main.tex` und `cover.tex`) und die einzige projektspezifische Stelle; `cover.tex` und die PDF-Metadaten ziehen sich alles von dort. Unbekannte Felder bleiben Platzhalter und werden als OFFEN gemeldet – `check_formalia.py chapters/ pages/` listet jeden verbliebenen im Audit namentlich (META-PLATZHALTER).
   ...
   **Regeln dabei:**
   ...
   - `\SubmissionDate` steht als Voreinstellung auf `\today` – **so gewollt** (Dauer-Präferenz des Nutzers, siehe `PERSISTENT.md`). Das Deckblatt trägt damit den Tag des letzten Builds. Nicht als Platzhalter melden und nicht ungefragt ersetzen; nur wenn der Nutzer ein festes Abgabedatum nennt, dieses eintragen.
